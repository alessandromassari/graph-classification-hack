...
from losses import FocalLoss,  compute_recon_loss
...
# PROX FUNZIONI DI LOSS DA CANCELLARE 
...
def pretraining(model, td_loader, optimizer, device, kl_weight_max, cur_epoch, an_ep_kl):
    ...
    for data in td_loader:
        ...
        reconstruction_loss = compute_recon_loss(z, data.edge_index, data.edge_attr,model.edge_attr_decoder, model.decoder)
...
def train(model, td_loader, optimizer, device, kl_weight_max, cur_epoch, an_ep_kl):
    ...
    for data in td_loader:
        ...
        adj_pred, edge_attr_pred, mu, logvar, class_logits, z = model(data, enable_classifier=True)
